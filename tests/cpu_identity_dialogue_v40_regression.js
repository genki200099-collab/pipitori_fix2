...
(async()=>{
  ...
  assert.strictEqual(new Set(avatars).size,4,'CPU animal icons are unique in one room');
  assert(names.every(name=>!/^CPU\d+$/i.test(name)),'internal CPU slot names never become display names');
  assert(latest.players.every(p=>p.displayIdentityKey&&p.displayIdentityKey===`${p.name}:${p.avatar}`),'name and icon are persisted as one identity pair');
  ...
  console.log(JSON.stringify({result:'passed',suite:'cpu-identity-dialogue-v40',cpuCount:4,names,avatars,personalitySeparated:true,commentDomStable:true}));
