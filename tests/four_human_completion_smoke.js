...
const middlePick=process.env.MIDDLE_PICK==='1';
...
function actionFor(c,state){
 ...
 assert.strictEqual(state.enableMiddleRankPick,middlePick);
 ...
 if(state.phase==='finished'){
  if(c.index===0)done(null,{result:'passed',suite:'four-human-completion',middlePick,elapsedMs:Date.now()-started,totals:state.players.map(p=>p.final.total),round:state.round});
  return;
 }
 ...
 if(pp?.targetSelectionRequired&&!pp.targetSelectionDone&&pp.pickProviderPid===state.yourIndex){
  ...
  if(ids.length&&c.last!==key){c.last=key;send(c.ws,{type:'pickTargets',pickId:pp.pickId,cardIds:ids});}return;
 }
 if(pp?.pairChoice&&pp.pickerPid===state.yourIndex){const key=`pair:${pp.pickId}:${pp.pairChoice.drawn?.id}`;if(c.last!==key){c.last=key;send(c.ws,{type:'pairChoice',pickId:pp.pickId,skip:true});}return;}
 if(pp&&!pp.result&&pp.pickerPid===state.yourIndex&&pp.ready){const key=`pick:${pp.pickId || pp.readyAt}`;if(c.last!==key){c.last=key;send(c.ws,{type:'pick',pickId:pp.pickId,index:0});}return;}
...
function connect(index,name,join=false){
 ...
 ws.on('open',()=>{if(join)send(ws,{type:'join',code,name});else send(ws,{type:'create',name,rounds:1,roundDealMode:'reshuffle',feastPointPerCard:2,pickProviderRole:'weakest',enableMiddleRankPick:middlePick});});
